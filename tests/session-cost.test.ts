...
describe('computeSessionCost', () => {
  ...
  it('applique le tarif par modèle (opus + fable-5 + fable-5.1 + opus-5.5)', () => {
    const projectsDir = makeTempDir();
    const mainPath = writeSessionFile(projectsDir, '-proj', 'sess1', [
      ...
      // Opus 5.5 : 1 M de lecture de cache à 0,20 $, contre 0,50 $ sur Opus 5.
      assistantEvent({ id: 'm4', requestId: 'r4', model: 'claude-opus-5-5', usage: { cache_read_input_tokens: 1_000_000 } }), // 0,20 $
    ]);
    const r = computeSessionCost({ transcriptPath: mainPath, sessionId: 'sess1', cwd: '/x', projectsDir, resolver });
